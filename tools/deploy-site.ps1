...
function CreateSite() {
	...
	if (Test-Path $sitePath)
	{
		...
		else
		{
			...
			## Clear contents of $sitePath folder, preserving the localconfig directory.
			Get-ChildItem $sitePath -Exclude localConfig | Get-ChildItem -Recurse | Foreach-Object {Remove-Item -Recurse -path $_.FullName }
